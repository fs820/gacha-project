...
async function registerUser() {
    ...
    // Googleのサーバーからトークンをもらう
    let token = "";
    try {
        token = await grecaptcha.execute('6LciX0EtAAAAAN7-8usbYbLMohfeox-oSRaYQVmq', { action: 'register' });
    }
    catch (e) {
        msgArea.innerText = "reCAPTCHAの読み込みに失敗しました";
        return;
    }
    try {
        const response = await fetch("/register", {
            ...
            body: JSON.stringify({ username: user, password: pass, recaptcha_token: token })
        });
...
async function loginUser() {
    ...
    let token = "";
    try {
        token = await grecaptcha.execute('6LciX0EtAAAAAN7-8usbYbLMohfeox-oSRaYQVmq', { action: 'login' });
    }
    catch (e) {
        msgArea.innerText = "reCAPTCHAの読み込みに失敗しました";
        return;
